    ...
    private void parseTolarie(Document doc, Builder<DailyCardInfo> builder) throws IOException {
        ...
        for (Element element : values) {
            try {
                ...
                long skladem = Long.parseLong(innerValues.get(0).text().replaceAll(".*skladem (\\d+).*","$1"));
                long cena = Long.parseLong(innerValues.get(6).text().replaceAll("(\\d+).*","$1"));
...
//        List<DailyCardInfo> temple_garden = new TolarieLoader().sniffByEdition(CardEdition.KHANS_OF_TARKIR);
        List<DailyCardInfo> temple_garden = new TolarieLoader().sniffByEdition(CardEdition.GUILDS_OF_RAVNICA);
